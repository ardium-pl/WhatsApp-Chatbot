        ...
        async with aiomysql.create_pool(
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute("SELECT id FROM users WHERE whatsapp_number_id = ?", (float(sender_phone_number),))
                    result = await cur.fetchone()

                    # Insert query-answer pair of the given user
                    if result:
                        user_id = result[0]
                        await cur.execute("INSERT INTO queries (user_id, query, answer) VALUES (?, ?, ?)",
